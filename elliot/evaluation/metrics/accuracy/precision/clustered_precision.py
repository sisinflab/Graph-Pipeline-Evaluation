...
class clustered_Precision(BaseMetric):
    r"""
    Precision-measure
    ...
    def name(self):
        ...
        """
        return f"Precision_users:{self._user_clustering_name}"
    ...
    def process(self):
        ...
        for user_group, users in self._user_clustering.items():
            self._values_dict[user_group] = np.average([self.__user_precision(u_r, u, self._cutoff) for u, u_r in self._recommendations.items() if len(self._relevance.get_user_rel(u)) and u in users])
