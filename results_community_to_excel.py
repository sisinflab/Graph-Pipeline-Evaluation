...
parser.add_argument('--folder', type=str, default='bookcrossing_results_tabusampler')
...
for f in files:
    ...
    df.drop(['model', 'nDCG', 'Recall',
             'Precision', 'HR', 'MAR', 'MAP',
             'MRR', 'F1', 'ACLT', 'APLT', 'ARP',
             'PopREO', 'ItemCoverage', 'Gini'], axis=1, inplace=True)
    ...
    df.to_excel(f'results/{args.folder}/{f.split(".")[0]}_params.xls', index=None)
